...
class VcoModule(name: String, tune: Float = TUNE_C0): FactoryModule(name, ModuleType.VCO) {

    companion object{
        const val TUNE_C0 = 16.3516f
    }

    override val parameters: List<ModuleParameter> = listOf(
        ModuleParameter("tune", tune),
        ModuleParameter("lfo_mode", false)
    )
