...
class MixLinearWithLimitModule(name: String = "mixLinearWithLimit", limit: Float = 1f):
    FactoryModule(name, ModuleType.MixLinearWithLimit)
{
    override val parameters = listOf(
        ModuleParameter("limit", limit)
    )

    val a = registerInput("A")
    val b = registerInput("B")
    val cv = registerInput("cv")
    val output = registerOutput("out")

    val userCv = registerFloatUserInput("cv", -1f..1f)
}