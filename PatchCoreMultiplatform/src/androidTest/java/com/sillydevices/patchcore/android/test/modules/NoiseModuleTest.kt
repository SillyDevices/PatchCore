...
import com.sillydevices.patchcore.modules.NoiseModule
...
class NoiseModuleTest {
    ...
    fun testNoise() {
        ...
        class TestSynth: ModularSynth() {
            val noise by module(NoiseModule("noise"))
            override val defaultPatch: Patch = createPatch {
                patch(noise.output, monoOutput)
                patch(noise.pinkOutput, monoOutput)
            }
        }
