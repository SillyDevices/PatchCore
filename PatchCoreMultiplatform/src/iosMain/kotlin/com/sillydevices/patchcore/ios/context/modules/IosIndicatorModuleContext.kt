...
class IosIndicatorModuleContext(pointer: ModulePointer, contextFactory: ContextFactory):
    IndicatorModuleContext,
    IosModuleContext(pointer, contextFactory)
{
    override fun setSize(newSize: Int) {
        setIndicatorBufferSize(getPointer().nativePointer, newSize)
    }

    override suspend fun copyInto(buffer: FloatArray, desiredCount: Int, startIndex: Int): Int {
        return copyIndicatorBuffer(
